...
describe('Authentication API Provider Verification', () => {
  ...
  const verifier = new Verifier({
    ...
    requestFilter: (req, res) => {
      ...
      if (req.path.includes('/api/auth/') && req.path !== '/api/auth/register' && req.path !== '/api/auth/login') {
    ...
    stateHandlers: {
      ...
      'user exists but credentials are invalid': async () => {
        ...
        console.log('Setting up state: user exists but credentials are invalid');
        return Promise.resolve();
      },
