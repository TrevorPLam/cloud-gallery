...
class MockCryptoService {
  ...
  async hashPassword(password: string, options: {
    iterations?: number;
    memorySize?: number;
    parallelism?: number;
  } = {}): Promise<string> {
    ...
    const estimatedTime = baseTime * iterationFactor * memoryFactor * parallelismFactor;
  ...
  async verifyPassword(password: string, hash: string): Promise<boolean> {
    ...
    const estimatedTime = baseTime * iterationFactor * memoryFactor * parallelismFactor * 0.8;
    await new Promise(resolve => setTimeout(resolve, estimatedTime));
  ...
  async decrypt(encryptedData: {
    ...
    // Simulate decryption
    const plaintext = Buffer.from(encryptedData.ciphertext, 'base64').toString();
  ...
  async deriveKey(password: string, salt: string, iterations: number = 100000): Promise<string> {
    ...
    const key = Buffer.from(`${password}:${salt}:${iterations}`).toString('base64');
  ...
  async generateToken(length: number = 32): Promise<string> {
    ...
    // Generate mock token
    const token = Buffer.from(length.toString()).toString('base64').slice(0, length);
...
describe('Cryptographic Operations Performance Tests', () => {
  ...
  describe('Password Hashing Performance', () => {
    ...
    bench('batch password hashing 10 passwords', async () => {
      ...
      console.log(`Batch password hashing: ${totalTime.toFixed(2)}ms total, ${avgTime.toFixed(2)}ms avg, ${throughput.toFixed(2)} ops/sec`);
    });
  });
  ...
  describe('Encryption/Decryption Performance', () => {
    ...
    bench('decrypt small data', async () => {
      const encrypted = await cryptoService.encrypt(smallData, 'test_key_123456');
      await PerformanceAssertions.assertTimeThreshold(
    ...
    bench('decrypt medium data', async () => {
      const encrypted = await cryptoService.encrypt(mediumData, 'test_key_123456');
      await PerformanceAssertions.assertTimeThreshold(
        () => cryptoService.decrypt(encrypted, 'test_key_123456'),
    ...
    bench('decrypt large data', async () => {
      const encrypted = await cryptoService.encrypt(largeData, 'test_key_123456');
      await PerformanceAssertions.assertTimeThreshold(
        () => cryptoService.decrypt(encrypted, 'test_key_123456'),
    ...
    bench('encrypt/decrypt roundtrip', async () => {
      ...
      if (totalTime > sharedThresholds.crypto.encrypt.maxTime * 4) {
        throw new Error(`Encrypt/decrypt roundtrip too slow: ${totalTime.toFixed(2)}ms`);
      }
  ...
  describe('HMAC Performance', () => {
    ...
    bench('batch HMAC calculation 50 items', async () => {
      ...
      console.log(`Batch HMAC calculation: ${totalTime.toFixed(2)}ms total, ${throughput.toFixed(2)} ops/sec`);
    });
  });
  ...
  describe('Token Generation Performance', () => {
    ...
    bench('batch token generation 100 tokens', async () => {
      ...
      console.log(`Batch token generation: ${totalTime.toFixed(2)}ms total, ${throughput.toFixed(2)} tokens/sec`);
    });
  });
  ...
  describe('Memory Usage Tests', () => {
    bench('memory usage during encryption', async () => {
      ...
      console.log(`Encryption memory usage: ${(memoryDelta / 1024 / 1024).toFixed(2)}MB`);
    });
    ...
    bench('memory usage during password hashing', async () => {
      ...
      console.log(`Password hashing memory usage: ${(memoryDelta / 1024 / 1024).toFixed(2)}MB`);
    });
  });
  ...
  describe('Security Performance Trade-offs', () => {
    bench('compare hashing parameters', async () => {
      ...
      results.forEach(r => {
        console.log(`  ${r.name}: ${r.duration.toFixed(2)}ms (m=${r.memorySize}, t=${r.iterations}, p=${r.parallelism})`);
      });
      
      // Verify that more secure configurations take longer
      const fastTime = results.find(r => r.name === 'fast')?.duration || 0;
      const secureTime = results.find(r => r.name === 'secure')?.duration || 0;
    ...
    bench('encryption throughput vs data size', async () => {
      ...
      results.forEach(r => {
        console.log(`  ${(r.size / 1024).toFixed(1)}KB: ${r.duration.toFixed(2)}ms, ${(r.throughput / 1024).toFixed(1)}KB/sec`);
      });
      ...
      if (avgThroughput < minThroughput) {
        throw new Error(`Encryption throughput too low: ${(avgThroughput / 1024).toFixed(1)}KB/sec`);
      }
    });
  });
});
