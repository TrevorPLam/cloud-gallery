...
describe("Search Performance Tests", () => {
  ...
  describe("Batch Search Performance", () => {
    ...
    bench("batch search 50 queries", async () => {
      ...
      console.log(
        `Batch search 50 queries: ${totalTime.toFixed(2)}ms total, ${avgTime.toFixed(2)}ms avg, ${throughput.toFixed(2)} ops/sec`,
      );
    });
    ...
    bench("concurrent search operations", async () => {
      ...
      console.log(
        `Concurrent search 20 queries: ${totalTime.toFixed(2)}ms total, ${throughput.toFixed(2)} ops/sec`,
      );
    });
  });
  ...
  describe("Search Scalability Tests", () => {
    ...
    bench("search performance with 5K photos", async () => {
      ...
      if (duration > expectedMaxTime) {
        throw new Error(
          `Search scaling issue: ${duration.toFixed(2)}ms > ${expectedMaxTime}ms`,
        );
      }
    ...
    bench("search memory usage", async () => {
      ...
      console.log(
        `Search indexing memory usage: ${(memoryDelta / 1024 / 1024).toFixed(2)}MB`,
      );
      searchService.clear();
    });
  });
