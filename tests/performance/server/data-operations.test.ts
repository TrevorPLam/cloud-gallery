...
class MockDatabaseService {
  ...
  async batchInsert(table: string, records: any[]) {
    ...
    await new Promise((resolve) =>
      setTimeout(resolve, 10 + records.length * 0.1),
    );
  ...
  async transaction(operations: { type: string; data: any }[]) {
    const startTime = performance.now();
    const results = [];
...
describe("Data Operations Performance Tests", () => {
  ...
  describe("Basic Database Queries", () => {
    ...
    bench("single photo query", async () => {
      await PerformanceAssertions.assertTimeThreshold(
        () =>
          dbService.query("SELECT * FROM photos WHERE id = ?", [
            "perf_photo_0",
          ]),
    ...
    bench("single album query", async () => {
      await PerformanceAssertions.assertTimeThreshold(
        () =>
          dbService.query("SELECT * FROM albums WHERE id = ?", [
            "perf_album_0",
          ]),
        serverThresholds.dataOperations.databaseQuery.maxTime,
  ...
  describe("Batch Operations Performance", () => {
    ...
    bench("batch insert 1000 photos", async () => {
      ...
      if (result.executionTime > expectedMaxTime) {
        throw new Error(
          `Batch insert 1000 photos too slow: ${result.executionTime.toFixed(2)}ms`,
        );
      }

      console.log(
        `Batch insert 1000 photos: ${result.executionTime.toFixed(2)}ms`,
      );
    });
    ...
    bench("batch insert with throughput check", async () => {
      ...
      const minThroughput =
        serverThresholds.dataOperations.batchInsert.minThroughput || 100;
      if (throughput < minThroughput) {
        throw new Error(
          `Batch insert throughput too low: ${throughput.toFixed(2)} ops/sec < ${minThroughput} ops/sec`,
        );
      }

      console.log(
        `Batch insert throughput: ${throughput.toFixed(2)} records/sec`,
      );
    });
  });
  ...
  describe("Data Export Performance", () => {
    ...
    bench("export JSON format (large dataset)", async () => {
      ...
      if (
        ...
        serverThresholds.dataOperations.dataExport.maxTime
      ) {
        throw new Error(
          `Large JSON export too slow: ${result.executionTime.toFixed(2)}ms`,
        );
      }

      console.log(
        `JSON export: ${result.recordCount} records, ${(result.size / 1024).toFixed(2)}KB, ${result.executionTime.toFixed(2)}ms`,
      );
    });
    ...
    bench("export CSV format", async () => {
      ...
      if (
        ...
        throw new Error(
          `CSV export too slow: ${result.executionTime.toFixed(2)}ms`,
        );
      }

      console.log(
        `CSV export: ${result.recordCount} records, ${(result.size / 1024).toFixed(2)}KB, ${result.executionTime.toFixed(2)}ms`,
      );
    });
    ...
    bench("export with filters", async () => {
      ...
      if (
        ...
        throw new Error(
          `Filtered export too slow: ${result.executionTime.toFixed(2)}ms`,
        );
      }

      console.log(
        `Filtered export: ${result.recordCount} records, ${result.executionTime.toFixed(2)}ms`,
      );
    });
  });
  ...
  describe("Memory Usage Tests", () => {
    bench("memory usage during batch operations", async () => {
      ...
      console.log(
        `Batch insert memory usage: ${(memoryDelta / 1024 / 1024).toFixed(2)}MB`,
      );
    });
    ...
    bench("memory usage during export", async () => {
      ...
      console.log(
        `Data export memory usage: ${(memoryDelta / 1024 / 1024).toFixed(2)}MB`,
      );
    });
  });
  ...
  describe("Scalability Tests", () => {
    bench("query performance scaling", async () => {
      ...
      if (performanceFactor > scaleFactor * 2) {
        throw new Error(
          `Query performance scaling poorly: ${performanceFactor.toFixed(2)}x slower for ${scaleFactor}x more data`,
        );
      }

      console.log(
        "Query scaling results:",
        results.map((r) => `${r.size}: ${r.duration.toFixed(2)}ms`).join(", "),
      );
    });
  });
});
