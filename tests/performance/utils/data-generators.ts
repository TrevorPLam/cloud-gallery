 ...
 */
export function generateTestPhotos(count: number, options: {
  minSize?: number;
  maxSize?: number;
  ...
  const tags = ['vacation', 'family', 'nature', 'city', 'portrait', 'landscape', 'sunset', 'beach', 'mountain', 'food', 'travel', 'wildlife', 'architecture', 'street', 'night'];
  const locations = [
  ...
  return Array.from({ length: count }, (_, i) => {
    const width = minDimensions[0] + Math.random() * (maxDimensions[0] - minDimensions[0]);
    const height = minDimensions[1] + Math.random() * (maxDimensions[1] - minDimensions[1]);
    ...
    if (includeMetadata) {
      basePhoto.metadata = {
        ...
        camera: {
          ...
          model: 'EOS R5',
          iso: 100 + (i % 3200),
          aperture: `f/${1.4 + (i % 8) * 0.4}`,
          shutterSpeed: `1/${100 + (i % 1000)}`,
          focalLength: `${24 + (i % 200)}mm`,
        },
        fileFormat: 'JPEG',
        colorSpace: 'sRGB',
        hasAlpha: false,
 ...
 */
export function generateTestAlbums(count: number, photosPerAlbum: number = 10): Album[] {
  const albumNames = [
  ...
  return Array.from({ length: count }, (_, i) => ({
    ...
    photoIds: Array.from({ length: photosPerAlbum }, (_, j) => `perf_photo_${i * photosPerAlbum + j}`),
...
export function generateSearchQueries(count: number) {
  const baseTerms = ['vacation', 'family', 'nature', 'city', 'portrait', 'landscape', 'sunset', 'beach', 'mountain', 'food', 'travel', 'wildlife', 'architecture', 'street', 'night'];
  const qualifiers = ['2023', '2024', 'summer', 'winter', 'spring', 'fall', 'new', 'old', 'favorite', 'recent'];
 ...
 */
export function generateBatchOperations(count: number, operationType: 'add' | 'update' | 'delete' = 'add') {
  return Array.from({ length: count }, (_, i) => ({
    type: operationType,
  ...
  return {
    ...
    complexQueries: Array.from({ length: 100 }, (_, i) => ({
      ...
      filters: {
        ...
        favorites: i % 3 === 0,
        albums: Array.from({ length: 5 }, (_, j) => `perf_album_${(i + j) % 50}`),
      },
      limit: 100,
      offset: (i % 10) * 100,
