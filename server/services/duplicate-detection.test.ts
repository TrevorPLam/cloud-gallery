  ...
  db: {
    ...
    insert: vi.fn().mockReturnValue({
      values: vi.fn().mockReturnValue({
        returning: vi
          .fn()
          .mockReturnValue(Promise.resolve([{ id: "test-id" }])),
      }),
    }),
...
describe("Duplicate Detection Service - Unit Tests", () => {
  describe("calculateHammingDistance", () => {
    ...
    it("should return correct distances for known cases", () => {
      ...
      expect(
        calculateHammingDistance("0000000000000000", "FFFFFFFFFFFFFFFF"),
      ).toBe(64);

      // Zero distance case (identical)
      expect(
        calculateHammingDistance("1234567890ABCDEF", "1234567890ABCDEF"),
      ).toBe(0);

      // Some bits different
      expect(
        calculateHammingDistance("0000000000000000", "0000000000000001"),
      ).toBe(1);
      expect(
        calculateHammingDistance("FFFFFFFFFFFFFFFF", "FFFFFFFEFFFFFFFF"),
      ).toBe(1);
    });
  });
  ...
  describe("updateDuplicateGroups - Integration Tests", () => {
    ...
    it("should handle non-existent photos gracefully", async () => {
      ...
      await expect(
        updateDuplicateGroups("test-user", "nonexistent-photo"),
    ...
    it("should handle photos without hashes", async () => {
      ...
      await expect(
        updateDuplicateGroups("test-user", "photo-without-hash"),
      ).resolves.not.toThrow();
    });
  });
});
