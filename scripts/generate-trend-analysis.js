...
function generateTrendAnalysis(coverageDir, outputPath) {
  try {
    ...
    if (Object.keys(historicalData).length < CONFIG.minDataPoints) {
      console.log(`⚠️ Insufficient data for trend analysis (need ${CONFIG.minDataPoints}, have ${Object.keys(historicalData).length})`);
    ...
    trendAnalysis.overall.recommendations = generateRecommendations(trendAnalysis);
    ...
    console.log(`   Date Range: ${new Date(trendAnalysis.dateRange.start).toLocaleDateString()} - ${new Date(trendAnalysis.dateRange.end).toLocaleDateString()}`);
    console.log(`   Quality Score: ${(trendAnalysis.qualityScore * 100).toFixed(1)}%`);
  ...
  try {
    ...
    files.forEach(file => {
      try {
        ...
        console.warn(`Warning: Could not load metrics from ${file}:`, error.message);
      }
    });
...
function analyzeMetricTrend(sortedData, metric) {
  ...
  const timestamps = sortedData.map(item => new Date(item.timestamp).getTime());
...
function filterDataByTimeWindow(sortedData, days) {
  ...
  return sortedData.filter(item => new Date(item.timestamp).getTime() >= cutoffTime);
}
...
function generateInsights(trendAnalysis) {
  ...
  if (metrics.successRate?.trend === 'declining') {
    insights.push(`Success rate is declining by ${Math.abs(metrics.successRate.changePercent).toFixed(1)}%`);
  } else if (metrics.successRate?.trend === 'improving') {
    insights.push(`Success rate is improving by ${metrics.successRate.changePercent.toFixed(1)}%`);
  }
  
  // Execution time insights
  if (metrics.averageTestTime?.trend === 'increasing') {
    insights.push(`Average test execution time is increasing by ${metrics.averageTestTime.changePercent.toFixed(1)}%`);
  } else if (metrics.averageTestTime?.trend === 'declining') {
    insights.push(`Test execution time is improving (decreasing) by ${Math.abs(metrics.averageTestTime.changePercent).toFixed(1)}%`);
  }
  
  // Stability insights
  if (metrics.stabilityScore?.trend === 'declining') {
    insights.push(`Test stability is declining by ${Math.abs(metrics.stabilityScore.changePercent).toFixed(1)}%`);
  } else if (metrics.stabilityScore?.trend === 'improving') {
    insights.push(`Test stability is improving by ${metrics.stabilityScore.changePercent.toFixed(1)}%`);
  }
  
  // Test count insights
  if (metrics.totalTests?.trend === 'increasing') {
    insights.push(`Test coverage is growing with ${metrics.totalTests.change} new tests`);
  } else if (metrics.totalTests?.trend === 'declining') {
    insights.push(`Test count is decreasing by ${Math.abs(metrics.totalTests.change)} tests`);
  }
  
  // Volatility insights
  if (metrics.successRate?.volatility > 0.1) {
    insights.push('Success rate shows high volatility - investigate test stability');
  }
  
  if (metrics.averageTestTime?.volatility > 100) {
    insights.push('Test execution times are highly variable - check for performance issues');
  }
...
function generateRecommendations(trendAnalysis) {
  ...
  if (metrics.successRate?.trend === 'declining' && metrics.successRate?.changePercent < -5) {
    recommendations.push('🔧 Address declining success rate - review recent code changes');
  }
  
  // Performance recommendations
  if (metrics.averageTestTime?.trend === 'increasing' && metrics.averageTestTime?.changePercent > 10) {
    recommendations.push('⚡ Optimize slow tests - execution time is trending upward');
  }
  
  // Stability recommendations
  if (metrics.stabilityScore?.trend === 'declining') {
    recommendations.push('🔍 Investigate test stability issues - implement better isolation');
  }
  ...
  if (metrics.successRate?.volatility > 0.15) {
    recommendations.push('🎯 Reduce test volatility - implement consistent test environments');
  }
  
  if (recommendations.length === 0) {
    recommendations.push('✅ Test metrics look stable - continue current practices');
  }
...
function detectAnomalies(sortedData) {
  ...
  sortedData.forEach((item, index) => {
    ...
    if (successRate < avgSuccessRate - 0.2) { // 20% below average
      anomalies.push({
  ...
  sortedData.forEach((item, index) => {
    ...
    if (execTime > avgExecTime * 2) { // 2x above average
      anomalies.push({
...
function calculateQualityScore(trendAnalysis) {
  ...
  if (performance) {
    if (performance.trend === 'declining') score += 0.1; // Lower execution time is good
...
if (require.main === module) {
  ...
  if (args.length !== 2) {
    console.error('Usage: node generate-trend-analysis.js <coverage-dir> <output-json>');
    process.exit(1);
  }
...
module.exports = { generateTrendAnalysis, analyzeMetricTrend, calculateQualityScore };
