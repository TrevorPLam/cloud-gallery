  ...
  try {
    ...
    if (metrics.testSuites) {
      Object.entries(metrics.testSuites).forEach(([filePath, suiteMetrics]) => {
        ...
        const suiteAnalysis = analyzeSuiteForFlakiness(
          filePath,
          suiteMetrics,
          historicalData,
        );
        ...
        if (suiteAnalysis.flakyTests.length > 0) {
          flakyTestResults.analysis.flakyTests.push(
            ...suiteAnalysis.flakyTests,
          );
        }

        if (suiteAnalysis.suspiciousTests.length > 0) {
          flakyTestResults.analysis.suspiciousTests.push(
            ...suiteAnalysis.suspiciousTests,
          );
        }

        if (suiteAnalysis.stableTests.length > 0) {
          flakyTestResults.analysis.stableTests.push(
            ...suiteAnalysis.stableTests,
          );
        }
      });
    }
    ...
    if (metrics.slowestTests && metrics.slowestTests.length > 0) {
      const slowTestAnalysis = analyzeSlowTests(
        metrics.slowestTests,
        historicalData,
      );
      flakyTestResults.analysis.detectionMethods.timeBased.push(
        ...slowTestAnalysis,
      );
    }
    ...
    if (metrics.failedTests > 0) {
      ...
      flakyTestResults.analysis.detectionMethods.failureBased.push(
        ...failureAnalysis,
      );
    }
    ...
    if (Object.keys(historicalData).length > 0) {
      const historicalAnalysis = analyzeHistoricalFlakiness(
        metrics,
        historicalData,
      );
      flakyTestResults.analysis.detectionMethods.historicalBased.push(
        ...historicalAnalysis,
      );
    }
    ...
    flakyTestResults.analysis.detectionMethods.patternBased.push(
      ...patternAnalysis,
    );
    ...
    summary.flakyRate =
      metrics.totalTests > 0 ? summary.flakyCount / metrics.totalTests : 0;
    summary.overallStability = calculateOverallStability(
      flakyTestResults.analysis,
    );

    // Generate recommendations
    flakyTestResults.recommendations =
      generateFlakyTestRecommendations(flakyTestResults);

    // Risk assessment
    flakyTestResults.riskAssessment = assessFlakyTestRisk(
      flakyTestResults.analysis,
    );
    ...
    console.log(
      `   Flaky Tests: ${summary.flakyCount} (${(summary.flakyRate * 100).toFixed(1)}%)`,
    );
    console.log(`   Suspicious Tests: ${summary.suspiciousCount}`);
    console.log(`   Stable Tests: ${summary.stableCount}`);
    console.log(
      `   Overall Stability: ${(summary.overallStability * 100).toFixed(1)}%`,
    );

    if (summary.flakyCount > 0) {
      console.log(`\n⚠️ Flaky Tests Detected:`);
      flakyTestResults.analysis.flakyTests
        .slice(0, 5)
        .forEach((test, index) => {
...
function loadHistoricalData(currentMetricsPath) {
  ...
  try {
    ...
    console.warn(
      "Warning: Could not read historical data directory:",
      error.message,
    );
  }
...
function analyzeSuiteForFlakiness(filePath, suiteMetrics, historicalData) {
  ...
  if (
    ...
    analysis.suspiciousTests.length === 0
  ) {
...
function analyzeFailurePatterns(metrics, historicalData) {
  ...
  if (metrics.failedTests > 0) {
    const failureRate =
      metrics.totalTests > 0 ? metrics.failedTests / metrics.totalTests : 0;
...
function generateFlakyTestRecommendations(results) {
  ...
  if (summary.flakyCount > 0) {
    recommendations.push(
      `🔧 Fix ${summary.flakyCount} flaky tests to improve reliability`,
    );
  }

  if (summary.suspiciousCount > 5) {
    recommendations.push(
      "🔍 Investigate suspicious tests that may become flaky",
    );
  }

  if (summary.overallStability < 0.8) {
    recommendations.push(
      "⚠️ Overall test stability is below 80% - consider comprehensive test review",
    );
  }
  ...
  results.analysis.detectionMethods.timeBased.forEach((test) => {
    if (test.severity === "high") {
      recommendations.push(
        `⏱️ Optimize "${test.name}" - very slow execution indicates potential issues`,
      );
    }
  });
  ...
  results.analysis.detectionMethods.failureBased.forEach((test) => {
    if (test.failureRate > 0.5) {
      recommendations.push(
        `🚨 Critical: "${test.name}" has ${(test.failureRate * 100).toFixed(1)}% failure rate`,
      );
    }
  });
...
function assessFlakyTestRisk(analysis) {
  ...
  risk.score =
    analysis.summary.flakyCount * 0.4 +
...
if (require.main === module) {
  ...
  if (args.length !== 2) {
    console.error(
      "Usage: node detect-flaky-tests.js <metrics-json> <output-json>",
    );
    process.exit(1);
  }
