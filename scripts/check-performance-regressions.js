...
function analyzeRegressions(current, baseline) {
  ...
  for (const [testName, currentResult] of Object.entries(current)) {
    ...
    if (currentResult.duration && baselineResult.duration) {
      const timeIncrease =
        (currentResult.duration - baselineResult.duration) /
    ...
    if (currentResult.memoryDelta && baselineResult.memoryDelta) {
      const memoryIncrease =
        (currentResult.memoryDelta - baselineResult.memoryDelta) /
        baselineResult.memoryDelta;
    ...
    if (currentResult.throughput && baselineResult.throughput) {
      const throughputDecrease =
        (baselineResult.throughput - currentResult.throughput) /
      ...
      if (throughputDecrease > THRESHOLDS.throughputRegressionThreshold) {
        ...
        throughputDecrease < -THRESHOLDS.throughputRegressionThreshold
      ) {
        improvements.push({
          type: "throughput",
          test: testName,
...
function main() {
  try {
    ...
    if (!fs.existsSync(resultsPath)) {
      console.log(
        "⚠️ No performance results found - skipping regression analysis",
      );
      process.exit(0);
    }
    ...
    console.log(
      `📊 Analyzing ${Object.keys(currentResults).length} performance tests`,
    );
    ...
    const { regressions, improvements, totalTests } = analyzeRegressions(
      currentResults,
      baseline,
    );
    ...
    if (regressions.length > 0) {
      ...
      regressions.forEach((regression, index) => {
        ...
        if (regression.type === "time") {
          console.log(
            `      Time: ${regression.current.toFixed(2)}ms vs ${regression.baseline.toFixed(2)}ms (+${regression.increase.toFixed(1)}%)`,
          );
        } else if (regression.type === "memory") {
          console.log(
            `      Memory: ${(regression.current / 1024 / 1024).toFixed(2)}MB vs ${(regression.baseline / 1024 / 1024).toFixed(2)}MB (+${regression.increase.toFixed(1)}%)`,
          );
        } else if (regression.type === "throughput") {
          console.log(
            `      Throughput: ${regression.current.toFixed(2)} ops/sec vs ${regression.baseline.toFixed(2)} ops/sec (-${regression.decrease.toFixed(1)}%)`,
          );
        }
    ...
    if (improvements.length > 0) {
      ...
      improvements.forEach((improvement, index) => {
        console.log(
          `   ${index + 1}. ${improvement.test} (${improvement.type}): +${improvement.improvement.toFixed(1)}%`,
        );
      });
    }
    ...
    const isMainBranch =
      ...
      process.env.GITHUB_REF?.includes("master");
