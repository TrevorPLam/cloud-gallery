...
export class AccessibilityTester {
  ...
  static async testComponent(
    ...
    try {
      ...
      console.warn(
        "Accessibility test skipped - React Native component not compatible with axe-core",
      );
      return { violations: [] };
    }
  }
  ...
  static checkWCAGCompliance(results: any) {
    ...
    const moderate =
      ...
      0;
    const minor =
      results.violations?.filter((v: any) => v.impact === "minor").length || 0;
    ...
    const score =
      totalViolations === 0
        ...
        : Math.max(
            0,
            100 - critical * 25 - serious * 15 - moderate * 10 - minor * 5,
          );
