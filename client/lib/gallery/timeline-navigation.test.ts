...
function createTestPhotos(count: number, dateRange?: { start: Date; end: Date }): Photo[] {
  const photos: Photo[] = [];
  const now = Date.now();
  const dayMs = 24 * 60 * 60 * 1000;
...
describe("Timeline Navigation", () => {
  ...
  describe("createTimelineHierarchy", () => {
    ...
    it("should create multiple years for photos from different years", () => {
      const photos = [
        ...createTestPhotos(3, { start: new Date(2023, 6, 1), end: new Date(2023, 8, 31) }),
        ...createTestPhotos(2, { start: new Date(2022, 6, 1), end: new Date(2022, 8, 31) }),
      ];
    ...
    it("should handle any scale value correctly", () => {
      fc.assert(
        fc.property(fc.float({ min: 0, max: 2 }), (scale) => {
          ...
          expect(DEFAULT_ZOOM_LEVELS.some(zl => zl.level === level.level)).toBe(true);
