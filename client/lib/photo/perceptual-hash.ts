...
export class PerceptualHasher {
   ...
   */
  public async generateAverageHash(imageUri: string): Promise<PerceptualHashResult> {
    const startTime = Date.now();
  ...
  public async findDuplicates(
    ...
    threshold: number = PerceptualHasher.DEFAULT_THRESHOLD
  ): Promise<Array<{ group: string; images: string[]; similarity: number }>> {
    ...
    const groups: Array<{ group: string; images: string[]; similarity: number }> = [];
      ...
      if (currentGroup.length > 1) {
        ...
        for (let x = 0; x < currentGroup.length; x++) {
          for (let y = x + 1; y < currentGroup.length; y++) {
            const hashX = hashes.find(h => h.uri === currentGroup[x])?.hash.hash;
            const hashY = hashes.find(h => h.uri === currentGroup[y])?.hash.hash;
            if (hashX && hashY) {
  ...
  private applyDCT(imageData: number[][]): number[][] {
    ...
    const dct: number[][] = Array(size).fill(0).map(() => Array(size).fill(0));
  ...
  private extractLowFrequency(dct: number[][], width: number, height: number): number[] {
  ...
  private computeSSIM(image1: number[][], image2: number[][]): number {
    ...
    for (let y = 0; y <= size - windowSize; y += windowSize) {
      for (let x = 0; x <= size - windowSize; x += windowSize) {
        ...
        const sigma12 = this.computeWindowCovariance(window1, window2, mu1, mu2);
  ...
  private extractWindow(image: number[][], x: number, y: number, size: number): number[] {
    const window: number[] = [];
  ...
  private computeWindowCovariance(window1: number[], window2: number[], mean1: number, mean2: number): number {
    let sum = 0;
  ...
  private fallbackHash(imageUri: string, algorithm: "phash" | "dhash" | "average", startTime: number): PerceptualHashResult {
 ...
 */
export function quickCompare(hash1: string, hash2: string, threshold: number = 5): boolean {
