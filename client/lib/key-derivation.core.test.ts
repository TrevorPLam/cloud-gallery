...
vi.mock("argon2", () => ({
  argon2: {
    hash: vi.fn(async (password: string, options: any) => {
      ...
      const input =
        password + JSON.stringify(options.salt) + options.associatedData;
      const hash = Buffer.from(input)
        .toString("hex")
        .padEnd(64, "0")
        .slice(0, 64);
...
describe("Key Hierarchy Integration", () => {
  ...
  it("should maintain key hierarchy consistency", async () => {
    ...
    const fileKey1Again = await deriveSpecializedKey(
      mockMasterKey,
      KeyType.FILE,
      "file1",
    );
  ...
  it("should provide proper domain separation", async () => {
    ...
    const deviceKey = await deriveSpecializedKey(
      mockMasterKey,
      KeyType.DEVICE,
      "same-id",
    );
