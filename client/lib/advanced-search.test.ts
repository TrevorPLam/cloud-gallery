...
describe("Advanced Search", () => {
  ...
  describe("Execution Planning", () => {
    ...
    it("should create execution plan for AND query", () => {
      ...
      expect(
        plan.steps.some((step) => step.operation === "AND_OPERATION"),
      ).toBe(true);
    ...
    it("should create execution plan for OR query", () => {
      ...
      expect(plan.steps.length).toBeGreaterThan(1);
      expect(plan.steps.some((step) => step.operation === "OR_OPERATION")).toBe(
        true,
      );
    });
    ...
    it("should create execution plan for NOT query", () => {
      ...
      expect(plan.steps.length).toBeGreaterThan(1);
      expect(
        plan.steps.some((step) => step.operation === "NOT_OPERATION"),
      ).toBe(true);
    ...
    it("should estimate result counts accurately", () => {
      ...
      expect(plan.estimatedResults).toBeLessThanOrEqual(
  ...
  describe("Query Optimization", () => {
    ...
    it("should optimize AND queries by selectivity", () => {
      ...
      expect(optimized.root.children?.[0].weight).toBeLessThanOrEqual(
        optimized.root.children?.[1].weight,
      );
    });
    ...
    it("should optimize OR queries by selectivity", () => {
      ...
      expect(optimized.root.children?.[0].weight).toBeGreaterThanOrEqual(
        optimized.root.children?.[1].weight,
      );
    });
  ...
  describe("Performance Tests", () => {
    it("should handle complex queries efficiently", async () => {
      const query = buildComplexQuery(
        "beach AND (sunset OR vacation) NOT portrait",
      );
    ...
    it("should handle multiple concurrent queries", async () => {
      ...
      const promises = queries.map((query) =>
        executeComplexQuery(query, index, sseKey),
      );
