...
import { generateEncryptionKey, encryptData, decryptData, XCHACHA20_KEYBYTES } from "./encryption";
 ...
 */
function applyFrequencyProtection(encryptedData: Uint8Array, bucketSize: number): Uint8Array {
  try {
...
export function encryptSearchTerm(
  ...
  try {
    // Validate input
    if (!term || term.length < MIN_QUERY_LENGTH || term.length > MAX_QUERY_LENGTH) {
      throw new Error(`Search term must be ${MIN_QUERY_LENGTH}-${MAX_QUERY_LENGTH} characters`);
    }
    ...
    encryptedBuffer.set(new Uint8Array(ciphertext, 0, ciphertextLen[0]), SSE_NONCEBYTES);
    
    // Apply frequency protection
    const bucketSize = PADDING_BUCKET_SIZES[Math.floor(Math.random() * PADDING_BUCKET_SIZES.length)];
...
export function decryptSearchTerm(
  ...
  try {
    ...
    const decryptedTerm = Buffer.from(plaintext, 0, plaintextLen[0]).toString("utf8");
