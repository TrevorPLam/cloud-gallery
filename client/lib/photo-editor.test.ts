...
describe("PhotoEditor - Property Tests", () => {
  ...
  describe("Undo/Redo Idempotence Properties", () => {
    it("should satisfy undo idempotence property", async () => {
      ...
      await fc.assert(
        fc.asyncProperty(adjustmentArb, async (adjustments) => {
  ...
  describe("History Consistency Properties", () => {
        ...
        fc.asyncProperty(
          ...
          async ([adjustments, commandCount]) => {
            ...
            for (let i = 0; i < commandCount; i++) {
              ...
              expect(testEditor.getHistoryIndex()).toBe(
    ...
    it("should clear redo history on new command", async () => {
      await fc.assert(
        fc.asyncProperty(adjustmentArb, async (adjustments1) => {
          const testEditor = createPhotoEditor(mockUri);
          ...
          if (testEditor.canUndo()) {
            ...
            const adjustments2 = {
              ...adjustments1,
              brightness: adjustments1.brightness + 0.1,
            };
            await testEditor.applyAdjustments(adjustments2);

            expect(testEditor.canRedo()).toBe(false);
            expect(testEditor.getHistory().length).toBeLessThanOrEqual(
              historyAfterFirst,
            );
          }
        }),
  ...
  describe("Filter Properties", () => {
    ...
    it("should satisfy original filter properties", () => {
      ...
      expect(
        adjustmentsEqual(originalFilter!.adjustments, DEFAULT_ADJUSTMENTS),
      ).toBe(true);
    });
  });
    ...
    it("should satisfy adjustment state consistency", async () => {
      await fc.assert(
        fc.asyncProperty(adjustmentArb, async (adjustments) => {
          ...
          expect(
            adjustmentsEqual(
              testEditor.getCurrentAdjustments(),
              DEFAULT_ADJUSTMENTS,
            ),
          ).toBe(true);
          ...
          await testEditor.applyAdjustments(adjustments);
          expect(
            adjustmentsEqual(testEditor.getCurrentAdjustments(), adjustments),
          ).toBe(true);
...
describe("PhotoEditor - Unit Tests", () => {
  ...
  describe("Basic Operations", () => {
    it("should create editor with correct initial state", () => {
      ...
      expect(
        adjustmentsEqual(editor.getCurrentAdjustments(), DEFAULT_ADJUSTMENTS),
      ).toBe(true);
    ...
    it("should apply adjustments correctly", async () => {
      ...
      expect(
        adjustmentsEqual(editor.getCurrentAdjustments(), adjustments),
      ).toBe(true);
    ...
    it("should apply filter correctly", async () => {
      ...
      expect(
        adjustmentsEqual(editor.getCurrentAdjustments(), filter!.adjustments),
      ).toBe(true);
    });

    it("should throw error for invalid filter", async () => {
      await expect(editor.applyFilter("invalid-filter")).rejects.toThrow(
        "Filter not found",
      );
    });
  });
  ...
  describe("Undo/Redo Operations", () => {
    it("should undo and redo correctly", async () => {
      const adjustments1: ImageAdjustments = {
        ...DEFAULT_ADJUSTMENTS,
        brightness: 0.5,
      };
      const adjustments2: ImageAdjustments = {
        ...DEFAULT_ADJUSTMENTS,
        contrast: 0.3,
      };
    ...
    it("should throw error when undo not possible", async () => {
      expect(editor.canUndo()).toBe(false);
      await expect(editor.undo()).rejects.toThrow(
    ...
    it("should throw error when redo not possible", async () => {
      expect(editor.canRedo()).toBe(false);
      await expect(editor.redo()).rejects.toThrow(
        "Cannot redo: no commands to redo",
      );
    });
  });
