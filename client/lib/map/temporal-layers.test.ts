...
describe('TimeUtils', () => {
  ...
  describe('Time Bucket Creation', () => {
    it('should create buckets correctly', () => {
      ...
      expect(buckets.every(bucket => bucket.count === bucket.photos.length)).toBe(true);
    });
    ...
    it('should respect padding', () => {
      ...
      const bucketsWithoutPadding = TimeUtils.createTimeBuckets(photos, 'day', 0);
      const bucketsWithPadding = TimeUtils.createTimeBuckets(photos, 'day', 5);
      
      expect(bucketsWithPadding.length).toBeGreaterThanOrEqual(bucketsWithoutPadding.length);
    });
  });
...
describe('TemporalLayersService', () => {
  ...
  describe('Time-based Photo Filtering', () => {
    ...
    it('should get photos at specific time', () => {
      ...
      const middleTime = timeRange.start + (timeRange.end - timeRange.start) / 2;
    ...
    it('should respect layer visibility', () => {
      ...
      if (layers.length > 0) {
        ...
        expect(visiblePhotos.length).toBeGreaterThanOrEqual(hiddenPhotos.length);
      }
    });
  });
  ...
  describe('Animation State Management', () => {
    ...
    it('should set animation time', () => {
      const timeRange = service.getOverallTimeRange();
      const targetTime = timeRange.start + (timeRange.end - timeRange.start) * 0.5;
    ...
    it('should calculate statistics correctly', () => {
      ...
      const layers: TemporalLayer[] = [
        ...
        },
        {
          id: 'layer-2',
          name: 'Layer 2',
          timeRange: { start: baseTime + 2 * 24 * 60 * 60 * 1000, end: baseTime + 3 * 24 * 60 * 60 * 1000 },
      ...
      fc.property(
        ...
        (timestamps, bucketSize) => {
          ...
          for (let i = 1; i < buckets.length; i++) {
            expect(buckets[i].start).toBeGreaterThanOrEqual(buckets[i - 1].start);
          }
...
describe('Performance Tests', () => {
  it('should handle large photo collections efficiently', () => {
    ...
    const timestamps = Array.from({ length: 1000 }, (_, i) => 
      baseTime + i * 24 * 60 * 60 * 1000 // One photo per day
    );
    const photos = createPhotosWithTimestamps(timestamps);
    ...
    bench('get photos at time', () => {
      const timeRange = service.getOverallTimeRange();
      const middleTime = timeRange.start + (timeRange.end - timeRange.start) * 0.5;
  ...
  it('should handle time bucketing efficiently', () => {
    ...
    const timestamps = Array.from({ length: 500 }, (_, i) => 
      baseTime + Math.random() * 365 * 24 * 60 * 60 * 1000
    );
    const photos = createPhotosWithTimestamps(timestamps);
