...
describe("WebMercatorProjection", () => {
  describe("Projection and Unprojection", () => {
    it("should project and unproject coordinates correctly", () => {
      ...
      const unprojected = WebMercatorProjection.unproject(
        projected.x,
        projected.y,
      );
  ...
  describe("Distance Calculation", () => {
    it("should calculate distances correctly", () => {
      const distance = WebMercatorProjection.distance(
        40.7128,
        -74.006,
        40.758,
        -73.9855,
  ...
  describe("Region Filtering", () => {
    it("should filter points by region correctly", () => {
      const photos = [
        ...
        createMockPhoto({
          location: { latitude: 34.0, longitude: -118.2, city: "LA" },
        }),
        createMockPhoto({
          location: { latitude: 51.5, longitude: -0.1, city: "London" },
        }),
      ];
      const points = renderer.processPhotos(photos);
  ...
  describe("Point Sampling", () => {
    ...
    it("should maintain geographic distribution", () => {
      ...
      const regions = new Set(
        sampled.map(
          (p) => `${Math.round(p.latitude)},${Math.round(p.longitude)}`,
        ),
      );
...
describe("Utility Functions", () => {
  describe("calculateOptimalRadius", () => {
    it("should calculate radius based on zoom level", () => {
      expect(calculateOptimalRadius(5)).toBeLessThan(
        calculateOptimalRadius(10),
      );
      expect(calculateOptimalRadius(15)).toBeGreaterThan(
        calculateOptimalRadius(10),
      );
    });
        ...
        (points, maxPoints) => {
          ...
          expect(
            sampled.every((p) =>
              points.some(
                (op) =>
                  op.longitude === p.longitude && op.latitude === p.latitude,
              ),
            ),
          ).toBe(true);
        },
...
describe("Performance Tests", () => {
  ...
  it("should handle projection calculations efficiently", () => {
    ...
    const projected = points.map((p) =>
      WebMercatorProjection.project(p.lat, p.lng),
    );
