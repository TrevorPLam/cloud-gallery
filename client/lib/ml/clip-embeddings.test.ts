...
describe('CLIPEmbeddingsService', () => {
  ...
  describe('Text Embeddings', () => {
    it('should generate text embeddings', async () => {
      ...
      const mockModelManager = (await import('./model-manager')).getModelManager();
    ...
    it('should tokenize text correctly', async () => {
      ...
      const mockModelManager = (await import('./model-manager')).getModelManager();
      mockModelManager.runInference.mockImplementation(async (modelName, inputs) => {
  ...
  describe('Image Embeddings', () => {
    ...
    it('should handle image generation errors gracefully', async () => {
      const mockModelManager = (await import('./model-manager')).getModelManager();
      mockModelManager.runInference.mockRejectedValue(new Error('Image processing failed'));

      const embeddings = await service.generateImageEmbeddings(['file:///test/image.jpg']);
  ...
  describe('Embedding Operations', () => {
    ...
    it('should find similar embeddings correctly', () => {
      ...
      const similarities = service.findSimilarEmbeddings(queryEmbedding, candidateEmbeddings, 3);
  ...
  describe('Model Management', () => {
    ...
    it('should handle invalid model name', async () => {
      await expect(service.switchModel('invalid-model')).rejects.toThrow('Unknown CLIP model');
    });
  ...
  describe('Error Handling', () => {
    it('should handle service not initialized error', async () => {
      ...
      await expect(uninitializedService.generateTextEmbeddings(['test']))
        .rejects.toThrow('CLIP service not initialized');
    });

    it('should handle embedding generation failures', async () => {
      const mockModelManager = (await import('./model-manager')).getModelManager();
      mockModelManager.runInference.mockRejectedValue(new Error('GPU delegate failed'));
  ...
  describe('Normalization', () => {
    it('should normalize embeddings correctly', () => {
      ...
      const normalizeEmbedding = (service as any).normalizeEmbedding.bind(service);
  ...
  describe('Integration with Model Manager', () => {
    it('should use correct delegate for iOS with Neural Engine', async () => {
      const mockTfliteManager = (await import('./tflite')).getTensorFlowLiteManager();
  ...
  describe('Memory Management', () => {
    it('should handle large embedding arrays efficiently', async () => {
      const largeTexts = Array(100).fill('large text query for testing memory efficiency');
