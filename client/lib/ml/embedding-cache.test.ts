...
describe('EmbeddingCache', () => {
  ...
  describe('Disk Cache Operations', () => {
    ...
    it('should respect disk cache size limits', async () => {
      ...
      const diskKeys = Array.from({ length: config.maxDiskEntries + 5 }, (_, i) => `@embedding_cache_disk-${i}`);
  ...
  describe('Encryption', () => {
    ...
    it('should decrypt data when retrieving from disk', async () => {
      ...
      mockDecryptData.mockResolvedValue(JSON.stringify({
        id,
        embedding: [0.1, 0.2, 0.3],
  ...
  describe('Progressive Generation', () => {
    beforeEach(() => {
      const mockClipService = (require('./clip-embeddings')).getCLIPEmbeddingsService();
      mockClipService.generateTextEmbeddings.mockResolvedValue([new Float32Array([0.1, 0.2, 0.3])]);
      mockClipService.generateImageEmbeddings.mockResolvedValue([new Float32Array([0.4, 0.5, 0.6])]);
    });
    ...
    it('should handle generation errors gracefully', async () => {
      ...
      const mockClipService = (require('./clip-embeddings')).getCLIPEmbeddingsService();
      mockClipService.generateTextEmbeddings.mockRejectedValue(new Error('Generation failed'));
      
      await expect(cache.generateAndCache(id, 'text', input, 'high'))
    ...
    it('should timeout for long-running generation', async () => {
      ...
      await expect(cache.generateAndCache(id, 'text', input, 'medium'))
  ...
  describe('Background Processing', () => {
    ...
    it('should process queue in FIFO order', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        promises.push(
          cache.generateAndCache(id, 'text', `query-${i}`, 'low')
      ...
      expect(processedOrder).toEqual(['queue-test-0', 'queue-test-1', 'queue-test-2']);
    });
  });
