...
describe('ModelRegistry', () => {
  ...
  describe('profile management', () => {
    ...
    it('should get profiles by use case', () => {
      const objectDetectionProfiles = registry.getProfilesByUseCase('object-detection');
      const faceDetectionProfiles = registry.getProfilesByUseCase('face-detection');
    ...
    it('should search profiles by query', () => {
      ...
      mobileResults.forEach(profile => {
        const searchText = `${profile.baseName} ${profile.description} ${profile.tags.join(' ')}`.toLowerCase();
...
describe('AdaptiveModelSelector', () => {
  ...
  describe('model selection', () => {
    ...
    it('should filter variants by constraints', async () => {
      ...
      await expect(selector.selectModel('mobilenet_v3', criteria)).rejects.toThrow();
    });
  ...
  describe('performance tracking', () => {
    ...
    it('should record performance metrics', () => {
      ...
      const history = selector.getPerformanceHistory('mobilenet_v3_small_quant');
  ...
  describe('performance estimation', () => {
    ...
    it('should estimate performance for iOS with Neural Engine', async () => {
      ...
      expect(result.expectedPerformance.batteryImpact).toMatch(/low|medium|high/);
      expect(result.expectedPerformance.thermalImpact).toMatch(/low|medium|high/);
    });

    it('should adjust performance based on delegate', async () => {
      const cpuResult = await selector.selectModel('mobilenet_v3', { delegate: 'none' });
      const gpuResult = await selector.selectModel('mobilenet_v3', { delegate: 'core-ml' });

      // GPU should be faster
      expect(gpuResult.expectedPerformance.inferenceTime).toBeLessThan(cpuResult.expectedPerformance.inferenceTime);
    });
        ...
        fc.assert(
          fc.asyncProperty(
            ...
            async (criteria) => {
              ...
              try {
                const result = await selector.selectModel('mobilenet_v3', criteria);
                const score = selector['scoreVariant'](result.selectedVariant, criteria);
        ...
        fc.assert(
          fc.asyncProperty(
            ...
            async (variant) => {
              ...
              expect(['low', 'medium', 'high']).toContain(performance.batteryImpact);
              expect(['low', 'medium', 'high']).toContain(performance.thermalImpact);
            }
          ),
          { numRuns: 15 }
        )
