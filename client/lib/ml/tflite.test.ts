...
describe("TensorFlowLiteManager", () => {
  ...
  describe("Inference", () => {
    ...
    it("should handle inference errors", async () => {
      ...
      vi.mocked(mockModel!.run).mockRejectedValue(
        new Error("Inference failed"),
      );

      await expect(
        manager.runInference("test-model", [new Uint8Array(224 * 224 * 3)]),
      ).rejects.toThrow("Inference failed");
    });

    it("should throw error for non-existent model", async () => {
      await expect(
        manager.runInference("non-existent", [new Uint8Array(224 * 224 * 3)]),
      ...
      await expect(
        fc.assert(
          fc.asyncProperty(fc.constant(null), async () => {
            ...
            expect(capabilities1?.supportedDelegates).toEqual(
              capabilities2?.supportedDelegates,
            );
          }),
          { numRuns: 5 },
        ),
...
describe("TensorFlowLiteManager Edge Cases", () => {
  ...
  it("should handle concurrent model loading", async () => {
    ...
    const configs: ModelConfig[] = [
      {
        name: "model1",
        path: "assets/models/model1.tflite",
        inputSize: 224,
        outputSize: 1000,
      },
      {
        name: "model2",
        path: "assets/models/model2.tflite",
        inputSize: 224,
        outputSize: 1000,
      },
      {
        name: "model3",
        path: "assets/models/model3.tflite",
        inputSize: 224,
        outputSize: 1000,
      },
    ];
