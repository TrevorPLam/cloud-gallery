...
describe('ModelManager', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(require('./tflite').getTensorFlowLiteManager).mockReturnValue(mockTfliteManager);
  });
  ...
  describe('Model Loading', () => {
    ...
    it('should handle concurrent loading requests', async () => {
      const promises = Array(3).fill(null).map(() => manager.loadModel(mockModelConfig));
    ...
    it('should handle loading errors', async () => {
      mockTfliteManager.loadModel.mockRejectedValue(new Error('Model not found'));
  ...
  describe('Inference', () => {
    ...
    it('should run inference successfully', async () => {
      ...
      expect(mockTfliteManager.runInference).toHaveBeenCalledWith('test-model', [new Uint8Array(224 * 224 * 3)]);
    });

    it('should load model if not loaded', async () => {
      mockTfliteManager.isModelLoaded.mockReturnValue(false);
      
      const result = await manager.runInference('test-model', [new Uint8Array(224 * 224 * 3)], mockModelConfig);
    ...
    it('should throw error for unloaded model without config', async () => {
      ...
      await expect(manager.runInference('unloaded-model', [new Uint8Array(224 * 224 * 3)]))
        .rejects.toThrow('Model "unloaded-model" not loaded and no config provided');
    });
  ...
  describe('Cache Management', () => {
    ...
    it('should unload all models', async () => {
      ...
      mockTfliteManager.isModelLoaded.mockReturnValue(true);
      mockTfliteManager.getLoadedModels.mockReturnValue(['test-model', 'test-model-2']);
    ...
    it('should optimize cache', async () => {
      ...
      // Simulate old models
      vi.spyOn(global, 'Date').mockImplementation(() => new Date('2023-01-15').getTime());
  ...
  describe('Configuration Management', () => {
    ...
    it('should handle configuration update errors gracefully', async () => {
      ...
      await expect(manager.updateConfig({ strategy: 'memory-only' })).resolves.toBeUndefined();
    });
  });
        ...
        fc.assert(
          fc.asyncProperty(
            ...
            async (maxMemoryMB) => {
              ...
              while (totalMemory < maxMemoryMB * 1024 * 1024 && modelsLoaded < 5) {
                const modelSize = Math.floor(Math.random() * 50 + 10) * 1024 * 1024; // 10-60MB
...
describe('ModelManager Edge Cases', () => {
  ...
  it('should handle memory pressure gracefully', async () => {
    ...
    await expect(lowMemoryManager.loadModel({
      name: 'large-model',
  ...
  it('should handle cleanup during operations', async () => {
    const mockTfliteManager = {
      ...
      loadModel: vi.fn().mockImplementation(() => new Promise(resolve => setTimeout(resolve, 100))),
      isModelLoaded: vi.fn().mockReturnValue(false),
      runInference: vi.fn(),
      getLoadedModels: vi.fn().mockReturnValue([]),
      getModelMetadata: vi.fn(),
  ...
  it('should handle invalid configurations', async () => {
    ...
    for (const config of invalidConfigs) {
      ...
      expect(['memory-only', 'disk-priority', 'adaptive', 'aggressive']).toContain(actualConfig.strategy);
    }
  });
});
