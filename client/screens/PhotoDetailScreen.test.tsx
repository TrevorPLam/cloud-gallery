...
describe('PhotoDetailScreen - Metadata Updates', () => {
  ...
  describe('Favorite Toggle Mutation', () => {
    it('should send PUT request with isFavorite field (Requirement 4.1)', async () => {
      const mockResponse = {
        json: vi.fn().mockResolvedValue({ photo: { id: 'photo-1', isFavorite: true } }),
        ok: true,
        status: 200,
      };
      ...
      expect(apiRequest).toHaveBeenCalledWith('PUT', '/api/photos/photo-1', { isFavorite: true });
    });
    ...
    it('should apply optimistic update immediately (Requirement 4.4)', async () => {
      ...
      queryClient.setQueryData(['photos'], [
        { id: 'photo-1', isFavorite: false, uri: 'test.jpg', width: 100, height: 100 },
      ]);
  ...
  describe('Tags Update Mutation', () => {
    it('should send PUT request with tags array (Requirement 4.2)', async () => {
      const mockResponse = {
        json: vi.fn().mockResolvedValue({ photo: { id: 'photo-1', tags: ['vacation', 'beach'] } }),
        ok: true,
        status: 200,
      };
      vi.mocked(apiRequest).mockResolvedValue(mockResponse as any);
      ...
      result.current.mutate({ photoId: 'photo-1', tags: ['vacation', 'beach'] });
    ...
    it('should update cache on success (Requirement 4.6)', async () => {
      const mockResponse = {
        json: vi.fn().mockResolvedValue({ photo: { id: 'photo-1', tags: ['vacation'] } }),
        ok: true,
        status: 200,
      };
      vi.mocked(apiRequest).mockResolvedValue(mockResponse as any);
  ...
  describe('Notes Update Mutation', () => {
    it('should send PUT request with notes field (Requirement 4.3)', async () => {
      const mockResponse = {
        json: vi.fn().mockResolvedValue({ photo: { id: 'photo-1', notes: 'Beautiful sunset' } }),
        ok: true,
        status: 200,
      };
      vi.mocked(apiRequest).mockResolvedValue(mockResponse as any);
  ...
  describe('Debouncing (Requirement 4.7)', () => {
    it('should debounce text input updates by 500ms', async () => {
      ...
      expect(mockMutate).toHaveBeenCalledWith({ photoId: 'photo-1', notes: 'Beautiful' });
