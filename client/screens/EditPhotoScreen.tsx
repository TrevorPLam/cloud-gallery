...
import { StyleSheet, View, Dimensions, ActivityIndicator, Alert, Pressable, ScrollView } from "react-native";
...
import { PhotoEditor, createPhotoEditor, ImageAdjustments, DEFAULT_ADJUSTMENTS } from "@/lib/photo-editor";
import { FILTER_PRESETS, getFilterPreset, FilteredImage } from "@/lib/filters/filter-system";
import { ADJUSTMENT_CONFIGS, getAdjustmentsByCategory, resetAdjustmentsToDefault } from "@/lib/adjustments";
...
export default function EditPhotoScreen() {
  ...
  const renderTabs = useMemo(() => (
    <View style={styles.tabContainer}>
      {(['filters', 'adjustments', 'tools', 'crop'] as EditorTab[]).map((tab) => (
        <Pressable
          key={tab}
          style={[
            styles.tab,
  ...
  }, [activeTab, renderFiltersTab, renderAdjustmentsTab, renderToolsTab, renderCropTab]);
    ...
    <ThemedView style={styles.container}>
      ...
      <View style={[styles.header, { paddingTop: insets.top }]}>
        ...
        <View style={styles.headerActions}>
          <Pressable onPress={handleUndo} disabled={!editor.canUndo()} style={styles.headerButton}>
            <Feather 
              name="rotate-ccw" 
              size={20} 
          ...
          <Pressable onPress={handleRedo} disabled={!editor.canRedo()} style={styles.headerButton}>
            <Feather 
              name="rotate-cw" 
              size={20} 
      ...
      <View style={[styles.bottomActions, { paddingBottom: insets.bottom }]}>
        <Pressable onPress={handleReset} style={styles.resetButton}>
          <ThemedText type="body" style={{ color: theme.textSecondary }}>Reset</ThemedText>
        </Pressable>
        <Pressable onPress={() => setShowBeforeAfter(!showBeforeAfter)} style={styles.compareButton}>
