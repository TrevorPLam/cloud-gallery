...
const INITIAL_NUM = Math.ceil(SCREEN_HEIGHT / (ITEM_HEIGHT + SEPARATOR_HEIGHT)) + 2;
...
const resolveDuplicates = async (resolutions: ResolutionRequest): Promise<{ resolved: number; total: number }> => {
...
const PhotoItem = React.memo<PhotoItemProps>(({ photo, isSelected, onToggle, onPress }) => {
    ...
    <Pressable style={styles.photoContainer} onPress={handlePress}>
      ...
      <View style={styles.photoInfo}>
        ...
        <ThemedText style={[styles.photoDetails]}>
          {photo.width}×{photo.height} • {(photo.fileSize / (1024 * 1024)).toFixed(1)}MB
        </ThemedText>
      ...
      <View style={styles.groupHeader}>
        <View style={styles.groupTitle}>
          ...
          <ThemedText style={[styles.groupTitleText]}>
            {group.groupType === 'exact' ? 'Exact Duplicates' : 
             group.groupType === 'burst' ? 'Burst Sequence' : 
             'Similar Photos'}
          </ThemedText>
        </View>
...
export default function DuplicatesScreen() {
  ...
  const handlePhotoPress = useCallback((photo: DuplicatePhoto) => {
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: () => {
            ...
            duplicatesData.duplicateGroups.forEach(group => {
              const groupSelectedPhotos = group.photos.filter(p => selectedPhotos.has(p.id));
              const groupUnselectedPhotos = group.photos.filter(p => !selectedPhotos.has(p.id));
              
              if (groupSelectedPhotos.length > 0 && groupUnselectedPhotos.length > 0) {
                resolutions.push({
            ...
            if (resolutions.length > 0) {
              ...
              Alert.alert('Invalid Selection', 'Please select at least one photo to keep in each group.');
            }
          },
        },
      ]
  ...
  const keyExtractor = useCallback((group: DuplicateGroup) => group.groupId, []);
  
  const renderGroup = useCallback(({ item }: { item: DuplicateGroup }) => (
    <DuplicateGroupItem
      group={item}
    ...
    <View style={[styles.header]}>
      ...
      {duplicatesData && (
        <View style={styles.headerStats}>
          ...
          <ThemedText style={[styles.headerStat]}>
            {duplicatesData.duplicateGroups.reduce((sum, group) => sum + group.photos.length, 0)} photos
          </ThemedText>
        </View>
      )}
    </View>
  ...
  const renderFooter = useMemo(() => {
    ...
    return (
      <View style={[styles.footer]}>
        ...
        >
          <ThemedText>Resolve {selectedPhotos.size > 0 ? `(${selectedPhotos.size})` : ''}</ThemedText>
        </Button>
      </View>
    );
  }, [theme, duplicatesData, selectedPhotos, handleResolve, resolveMutation.isPending]);
